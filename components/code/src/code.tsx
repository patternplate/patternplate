...
export const Code: React.SFC<CodeProps> = props => {
      ...
      {highlights.length > 0 && (
        <StyledLines>
          {props.children.split("\n").map((line, index) => (
            <StyledLine
              key={index}
              highlight={highlights.indexOf(index + 1) > -1}
            >
              {line || <span> </span>}
            </StyledLine>
          ))}
...
const StyledLine = styled.div<StyleLineProps>`
  position: relative;
  color: transparent;
...
const StyledLines = styled(StyledCode)`
  position: absolute;
  top: 0;
  right: 0;
  left: 0;
...
function highlightCode(language: string, source = "") {
  if (!language) {
    return source;
  }
  if (!source) {
