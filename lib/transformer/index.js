...
function transformer(patternplate, next) {
	...
	executors.forEach(function(exec){
		var dirname = path.resolve(__dirname, '../../', patternplate.defaults.api, patternplate.defaults[exec].dir);
		var pattern = patternplate.defaults[exec].pattern;
		patternplate[exec] = expose(dirname, pattern);
	});

	// Require user executors (if any)
	executors.forEach(function(exec){
		var dirname = path.resolve(patternplate.config.paths.root, patternplate.config.api, patternplate.config[exec].dir);
		var pattern = patternplate.config[exec].pattern;
	...
	applyTransforms(patternplate.patterns, patternplate.intents, patternplate.transforms);
	next();
}
