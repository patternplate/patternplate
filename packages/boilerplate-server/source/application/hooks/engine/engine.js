...
function engineBlueprint() {
  ...
  return class Engine {
    ...
    async start(host, port) {
      ...
      if (application.runtime.env === 'development') {
        if ((await ports.test(port, host)) !== true) {
          if (server.autoPort !== true) {
            throw new Error(
              `Port ${port} is taken and server.autPort is disabled, could not start server.`
            );
          }

          application.log.warn(
            `Port ${port} is taken, trying to obtain next open port... `
          );
          server.port = await ports.find(
            server.port + 1,
            server.port + 51,
            server.host
          );
          ...
          application.subs.forEach(sub => {
            application.log.debug(
              `Changing configuration of subapplications ${sub.name}`
            );

            sub.mountable.configuration.server = server;
            sub.mountable.configuration.client = Object.assign(
              sub.mountable.configuration.client || {},
              {
                host: server.host,
                port: server.port
              }
            ...
            application.log.silly(
              `${sub.mountable.name}.configuration.server: ${JSON.stringify(
                sub.mountable.configuration.server
              )}`
            );
            application.log.silly(
              `${sub.mountable.name}.configuration.client: ${JSON.stringify(
                sub.mountable.configuration.client
              )}`
            );
          });
      ...
      application.log.debug(
        `Starting engine at http://${server.host}:${server.port} in environment '${env}' ...`
      );
    ...
    mount(mountable, path = '/') {
      ...
      if (path === '/') {
        mountable.router.stack.routes.forEach(route => {
          ...
          if (match) {
            ...
            application.log.debug(
              `Route "${route.name}" of "${mountable.name}" overwrites ${application.name}'s route with same name.`
            );
          }
        });

        application.router.stack.routes = application.router.stack.routes.concat(
          mountable.router.stack.routes
        );
      } else {
      ...
      application.router.stack.middleware.forEach(middleware => {
        const [match] = mountable.router.stack.middleware.filter(
          mountMiddleware => mountMiddleware.name === middleware.name
        );
        if (match) {
          return;
        }
      ...
      mountable.configuration.middlewares =
        mountable.configuration.middlewares || {};

      // Override middleware config on mountable by host middleware config
      for (const middlewareName of Object.keys(
        application.configuration.middlewares.enabled || {}
      )) {
        const config =
          application.configuration.middlewares.enabled[middlewareName];
        let mountableConfig =
      ...
      mountable.configuration.client = {
        ...mountable.configuration.client,
        ...application.configuration.server
      };

      application.log.debug(
        `Changing configuration of subapplications ${mountable.name}`
      );
      application.log.silly(
        `${mountable.name}.configuration.server: ${JSON.stringify(
          mountable.configuration.server
        )}`
      );
      application.log.silly(
        `${mountable.name}.configuration.client: ${JSON.stringify(
          mountable.configuration.client
        )}`
      );
