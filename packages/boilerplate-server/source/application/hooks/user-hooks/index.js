...
export default {
  ...
  async start(application) {
    const coreHookPath = resolve(
      ...
      application.configuration.paths.hooks
    );
    ...
    for (const userHookPath of userHookPaths) {
      ...
      try {
        ...
        application.log.debug(
          `Loaded ${loadedHooks.length} user hooks: ${loadedHooks.map(
            loadedHook => loadedHook.name
          )}`
        );
      } catch (error) {
        application.log.error(
          `Failed loading hooks from ${userHookPath}: ${error.message}`
        );
        if (error.stack) {
    ...
    userHooks = userHooks
      .map(userHook => {
        ...
        const conflictingCoreHook = application.hooks.filter(
          coreHook => coreHook.name === userHook.name
        )[0];
        if (conflictingCoreHook) {
          throw new Error(
            `Hook "${userHook.name}" from ${userHook.requirePath} conflicts with core hook "${conflictingCoreHook.name}", will not load.`
          );
        }
        return userHook;
      })
    ...
    const registered = [...application.hooks, ...userHooks].map(hook =>
      hook.register(application)
    );

    const jobs = runHookTree(
      getHookTree(registered),
      registered,
      application,
      {}
    );
