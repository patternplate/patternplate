  ...
  start: async function startMiddlewareHook(application) {
    const coreMiddlewares = requireAll({
      dirname: resolve(
        ...
        application.configuration.paths.middlewares
      ),
      filter: /^([^.].*)\.js(on)?$/,
    ...
    const middlewarePaths = this.configuration.path.reduce(
      (items, item) =>
        items.concat(application.runtime.cwds.map(cwd => resolve(cwd, item))),
      []
    );
    ...
    const moduleMiddlewares = Object.keys(this.configuration.enabled)
      ...
      .reduce((result, middlewareName) => {
        ...
        try {
          ...
          this.log.silly(
            `Required module middleware '${middlewareName}' from module '${middlewareModuleName}'`
          );
        } catch (err) {
          this.log.warn(
            `Could not require module middleware '${middlewareName}' from module '${middlewareModuleName}'`
          );
          this.log.error(err);
          throw err;
        }
    ...
    Object.keys(middlewares).forEach(middlewareName => {
      ...
      if (
        ...
        (isObject && middlewareConfig.enabled !== true)
      ) {
        this.log.debug(
          `Middleware '${middlewareName}' is explicitly disabled.`
        );
        return;
      }
