    ...
    const moduleTasks = Object.keys(this.configuration)
      ...
      .reduce((result, taskName) => {
        ...
        try {
          ...
          this.log.debug(
            `Required module route '${taskName}' from module '${taskModuleName}'`
          );
        } catch (err) {
          this.log.warn(
            `Could not require module route '${taskName}' from module '${taskModuleName}'`
          );
          this.log.debug(err);
        }
