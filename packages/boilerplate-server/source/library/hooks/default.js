...
class Hook {
  ...
  register(application) {
    ...
    const matchesModes =
      hasModes && this.modes.indexOf(application.runtime.mode) > -1;

    if (hasModes && !matchesModes) {
      this.log.debug(
        `Hook ${this.name} is disabled in mode ${application.runtime.mode}.`
      );
  ...
  async stage(stageName, application) {
    ...
    try {
      ...
      this.log.error(
        `An error ocurred on stage ${stageName} of hook '${this.name}'`
      );
      if (error.stack) {
