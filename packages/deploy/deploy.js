...
const surge = (args, opts) => {
  return execa("surge", args, opts);
};


const PROVIDERS = ['github', 'surge'];
...
async function main(cli) {
  ...
  const provider = PROVIDERS.includes(cli.flags.provider) ? cli.flags.provider : 'github';
  ...
  if (provider === 'surge') {
    if (!name) {
      console.log(`--name is required for --provider=surge`);
      return process.exit(1);
    }

    const safeDomain = `${name}.surge.sh`;
    await surge([cwd, safeDomain], {stdout: "inherit", stderr: "inherit"});
  }
}
