...
async function read(pattern, subPath) {
  ...
  pattern.log.silly(
    `Listed ${fileList.length} files for ${pattern.id} ${fileListDuration}`
  );

  // Use filter, use all formats if none given
  const inFormats =
    pattern.filters.inFormats.length > 0
  ...
  const outFormats = inFormats
    .reduce((result, format) => {
      ...
      const transformOutFormats = transformNames
        .map(transformName => [
          transformName,
          pattern.config.transforms[transformName] || {}
        ])
    ...
    const formatNames = Object.entries(pattern.config.patterns.formats)
      .map(entry => {
        ...
        return transforms.indexOf(
          config.transforms[config.transforms.length - 1]
        ) > -1
          ? name
          : null;
      })
  ...
  const filteredFormats =
    pattern.filters.outFormats.length > 0 ? inOutFormats : inFormats;

  pattern.log.silly(
    `${pattern.id} has ${filteredFormats.length} formats available: ${chalk.grey(
      filteredFormats
    )}`
  );
  ...
  const matchingFilesList = chalk.grey(
    `[${matchingFiles.map(file => path.basename(file))}]`
  );
  pattern.log.silly(
    `Using ${matchingFiles.length} of ${files.length} files for ${pattern.id}: ${matchingFilesList}`
  );
  ...
  const fileData = await Promise.all(
    matchingFiles.map(
      throat(5, async file => {
        ...
        const transformNames = formatConfig.transforms || [];
        const transforms = transformNames.map(
          name => pattern.config.transforms[name] || {}
        );
        const resolveDependencies = transforms.some(
          transform => transform.resolveDependencies !== false
        );
        ...
        if (isRoot === false && resolveDependencies) {
          pattern.log.silly(
            `Reading ${pattern.id} as dependeny of ${pattern.config.parents[
              pattern.config.parents.length - 1
            ]}`
          );
        }
        ...
        if (fileRumpName === 'demo') {
          ...
          const overridden = Object.keys(demoDependencies).filter(
            key => key in dependencies
          );
