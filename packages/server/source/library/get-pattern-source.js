...
function getPatternSource(application) {
  ...
  return async (pathname, type, environment, options) => {
    ...
    const format =
      application.configuration.patterns.formats[inFormatName] || {};
    ...
    if (transforms.length && !transformName) {
      const error = new Error(
        `Could not determine last transform for "${basename}" of "${id}"`
      );
      error.status = 404;
      throw error;
    }
    ...
    if (transformName && !transform) {
      const error = new Error(
        `Transform "${transformName}" to be applied on "${basename}" of "${id}" is not configured`
      );
      error.status = 404;
      throw error;
    }
