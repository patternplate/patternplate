  ...
  return (
    environments
      .filter(userEnvironment => {
        envDebug(
          'using filters %s for environment %s to match against %s',
          userEnvironment.applyTo,
          userEnvironment.name,
          patternID
        );

        const positives = userEnvironment.applyTo.filter(
          glob => glob[0] !== '!'
        );
        ...
        const matchPositive = positives.filter(positive =>
          minimatch(patternID, positive)
        );

        const matchNegative = negatives.filter(negative =>
          minimatch(patternID, negative)
        );
        ...
        if (matchPositive.length > 0) {
          envDebug(
            'positive match for environment %s on %s: %s',
            userEnvironment.name,
            patternID,
            matchPositive
          );
        ...
        if (matchNegative.length > 0) {
          envDebug(
            'negative match for environment %s on %s: %s',
            userEnvironment.name,
            patternID,
            matchNegative
          );
