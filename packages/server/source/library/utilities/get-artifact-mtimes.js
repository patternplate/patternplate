...
export default async function getArtifactMtimes(search, patterns, transforms) {
  ...
  const types = Object.keys(patterns.formats).map(extension =>
    patterns.formats[extension].name.toLowerCase()
  );

  const typedFiles = await Promise.all(
  ...
  const artifactPaths = typedFiles.reduce(
    (flattened, files) => [...flattened, ...files],
    []
  );
