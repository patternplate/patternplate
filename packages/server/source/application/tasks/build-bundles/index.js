...
export default async (application, settings) => {
  ...
  const filterEnvironments = settings.env
  ...
  const envs = environments.filter(
    environment => environment.include && environment.include.length
  );

  spinner.stop();

  const envMaxLength = envs
    .map(e => e.name.length)
  ...
  await Promise.all(
    envs.map(
      throat(1, async environment => {
        ...
        // Write the bundle
        const writing = artifacts.map(
          throat(1, async entry => {
