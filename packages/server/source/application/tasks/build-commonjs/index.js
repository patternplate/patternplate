...
async function exportAsCommonjs(application, settings) {
  ...
  application.transforms = (await loadTransforms(settings.transforms || {}))(
    application
  );
  ...
  application.log.debug(
    ok`Read artifact modification times ${artifactMtimesStart}`
  );
  ...
  const pkgConfigChanged =
    !isEqual(previousPkgConfig, pkgConfig) ||
  ...
  let buildCount = 1;
  const patternFilter = pkgConfigChanged
    ? i => i
  ...
  const padMaxBuild = padEnd(
    patternsToBuild
      .map(pattern => pattern.id.length)
      .reduce((a, b) => (a > b ? a : b), 0) + 1
  );

  if (pkgConfigChanged) {
    application.log.debug(
      ok`Manifest or pkg config change, building all ${patternMtimes.length} patterns`
    );
  }
  ...
  const padMaxPrune = padEnd(
    artifactsToPrune
      .map(artifact => artifact.length)
      .reduce((a, b) => (a > b ? a : b), 0) + 1
  );

  application.log.debug(
    ok`Detected ${artifactsToPrune.length} artifacts to prune ${pruneDetectionStart}`
  );
    ...
    artifactsToPrune.map(
      throat(1, async path => {
        ...
        spinner.text = `prune ${padMaxPrune(
          path
        )} ${pruneCount}/${artifactsToPrune.length}`;
    ...
    patternsToBuild.map(
      throat(1, async pattern => {
        ...
        if (artifact) {
          ...
          // - their folder
          // - their pattern.json
          changedFiles = pattern.files
            .filter(file => {
              const formatKey = extname(file).slice(1);
              const format =
                application.configuration.patterns.formats[formatKey];
              if (!format) {
                return false;
              }
              const transformNames = format.transforms || [];
              const lastTransformName =
                transformNames[transformNames.length - 1];
              const lastTransform =
                application.configuration.transforms[lastTransformName] || {};
        ...
        if (artifact) {
          ...
          application.log.debug(
            ok`Building all files for ${pattern.id} ${filterStart}`
          );
        }
        ...
        spinner.text = `build ${padMaxBuild(
          pattern.id
        )} ${buildCount}/${patternsToBuild.length}`;
        buildCount += 1;
        ...
        application.log.debug(
          ok`Transformed pattern ${pattern.id} ${transformStart}`
        );
        ...
        const writingArtifacts = Promise.all(
          patternList.map(async patternItem => {
            const writingPatternItems = Promise.all(
              Object.entries(patternItem.results).map(async resultsEntry => {
                const [resultName, result] = resultsEntry;
                const relativePath = resolvePathFormatString(
                  pathFormatString,
                  patternItem.id,
        ...
        application.log.debug(
          ok`Wrote ${written.length} artifacts for ${pattern.id} ${writeStart}`
        );
        return patternList;
      })
    )
  ...
  application.log.debug(
    ready`Built ${built.length} from ${patternsToBuild.length} planned and ${patternMtimes.length} artifacts overall ${buildStart}`
  );
  ...
  if (application.resources) {
    ...
    await Promise.all(
      resources.map(async resource => {
        ...
        const resourcePath = resolvePathFormatString(
          pathFormatString,
          resource.pattern,
          formatConfig.name,
          resource.type
  ...
  const dependencies = dependencyLists.dependencies.reduce(
  ...
  const devDependencies = dependencyLists.devDependencies.reduce(
    (results, dependencyName) => {
      return {
        ...results,
        [dependencyName]: deps[dependencyName] || devDeps[dependencyName] || '*'
  ...
  const prunedDependencies = omit(dependencies, [
    ...(settings.ignoredDependencies || []),
    coreModuleNames
  ]);
  const prunedDevDependencies = omit(devDependencies, [
    ...(settings.ignoredDevDependencies || []),
    coreModuleNames,
    ...Object.keys(dependencies)
  ]);

  const updatedPackageString = getPackageString(
    prunedDependencies,
    previousPkg,
    {
