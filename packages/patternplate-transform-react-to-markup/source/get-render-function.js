...
export default (toStatic = true, application) => {
  ...
  if (wantsReactDOM && !reactDOMavailable) {
    const warning = [
      ...
      `React.${methodName} and moved it to react-dom/server's ${methodName},``but react-dom is not available via require.resolve.`, // eslint-disable-line no-unexpected-multiline
  ...
  const nameSpace =
    wantsReactDOM && reactDOMavailable ? require('react-dom/server') : React;
