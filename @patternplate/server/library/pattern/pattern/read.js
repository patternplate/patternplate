...
let read = (() => {
	var _ref = _asyncToGenerator(function* (pattern, subPath) {
		...
		const outFormats = inFormats.reduce(function (result, format) {
			...
			const transformOutFormats = transformNames.map(function (transformName) {
		...
		const inOutFormats = outFormats.reduce(function (result, format) {
			const transforms = Object.entries(pattern.config.transforms).map(function (entry) {
			...
			const formatNames = Object.entries(pattern.config.patterns.formats).map(function (entry) {
		...
		const files = fileList.filter(function (file) {
		...
		const out = files.map(function (file) {
		...
		const matchingFiles = files.filter(filterFiles).map(function (file) {
			return _path2.default.resolve(pattern.base, pattern.id, file);
		});

		const matchingFilesList = _chalk2.default.grey(`[${matchingFiles.map(function (file) {
		...
		const fileData = yield Promise.all(matchingFiles.map((0, _throat2.default)(5, (() => {
			var _ref2 = _asyncToGenerator(function* (file) {
				...
				const transformNames = formatConfig.transforms || [];
				const transforms = transformNames.map(function (name) {
					return pattern.config.transforms[name] || {};
				});
				const resolveDependencies = transforms.some(function (transform) {
					return transform.resolveDependencies !== false;
				});
				const isRoot = pattern.config.parents.length === 0;

				const fileContents = isRoot || resolveDependencies ? yield read(file) : new Buffer('', 'utf-8');
				...
				if (fileRumpName === 'demo') {
					...
					const overridden = Object.keys(demoDependencies).filter(function (key) {
		...
		pattern.files = fileData.reduce(function (results, data) {
			return _extends({}, results, { [data.name]: data });
		}, {});

		// expose which file to use for rendering
		const basenames = matchingFiles.map(function (matchingFile) {
...
var _constructDemoFileDependencies = require('./construct-demo-file-dependencies');
